...
<div class="user_titles"><p>Edit All The Users here <%=username %>!</p></div>
<table>
 ...
 <%  String[] users = (String[]) request.getAttribute("all_users");
   	for(int i = 0; i < users.length; i++){  
    		if (users[i]!=""){ %>
    			...
    			<td>
			    <ul>
					<li style="position:relative;top:14px;font-weight:bold;"><%=users[i]%></li><br>
				</ul>
			    </td>
    		    <form name = "delete_user" action="./AdminServlet" method="post"> 
				<td><input type="hidden" name="delete_user" value="<%=users[i]%>"  /></td>
				<td><input align="middle" type="submit" value="Delete this user" class="btn btn-default" style="position:relative;right:-9px;"></td> 
			 	</form> 
