...
<div class="user_titles"><p>Edit your uploads here <%=username %>!</p></div>
<table>
   <% if  (userAccount_log != null){ %>
 ...
 <% String[] movie = (String[]) request.getAttribute("movie");
 	...
 	<%	if (movie[i]!=""){ %>
		    ...
		    <form name = "delete_video" action="./RegisteredUserServlet" method="post"> 
			<td><input type="hidden" name="delete_video" value="<%=movie[i]%>" /></td>
	 ...
	 String[] movie = (String[]) request.getAttribute("movie");
   	for(int i = 0; i < movie.length; i++){  
    		if (movie[i]!=""){ %>
    		    ...
    		    <form name = "delete_video" action="./RegisteredUserServlet" method="post"> 
				<td><input type="hidden" name="delete_video_reg" value="<%=movie[i]%>" /></td>
				<td><input align="middle" type="submit" value="Delete this video" class="btn btn-default"></td> 
			 	</form> 
			 	</tr>
    <% } %>
	 ...
	 String[] movie = (String[]) request.getAttribute("movie");
   	for(int i = 0; i < movie.length; i++){  
    		if (movie[i]!=""){ %>
    		    ...
    		    <form name = "delete_video" action="./AdminServlet" method="post"> 
				<td><input type="hidden" name="delete_video" value="<%=movie[i]%>" /></td>
				<td><input align="middle" type="submit" value="Delete this video" class="btn btn-default"></td> 
			 	</form> 
			 	</tr>
